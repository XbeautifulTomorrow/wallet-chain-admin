  ...
  <div>
    <div class="searchBox">
      <div class="searchLeft">
        ...
        <el-select v-model="searchForm.businessesId" filterable clearable @change="fetchWalletWithdrawalList()"
          placeholder="全部平台">
          <el-option v-for="(item, index) in platformDrop" :key="index" :label="item.businessesName" :value="item.id">
          </el-option>
        </el-select>

        <el-select v-model="searchForm.chainName" filterable clearable @change="fetchWalletWithdrawalList()"
          placeholder="所有网络">
        ...
        <el-button size="medium" type="primary" icon="el-icon-search"
          @click="fetchWalletWithdrawalList()">搜索</el-button>
      </div>
    ...
    <el-table border :data="withdrawalList" height="73vh" style="width: 99%" @sort-change="sortChange">
      ...
      <el-table-column prop="userRecharge" label="总转出"></el-table-column>
      <el-table-column prop="gasAmount" label="转出次数"></el-table-column>
      <el-table-column prop="gasAggregationAmount" label="总GAS"></el-table-column>
...
export default {
  ...
  created() {
    ...
    this.fetchWalletWithdrawalList();
  ...
  methods: {
    ...
    async fetchWalletWithdrawalList(isSearch = true) {
      ...
      const res = await this.$http.getWalletWithdrawalList(data);
      if (res) {
    ...
    handleCurrentChange(val) {
      this.page = val;
      this.fetchWalletWithdrawalList(false);
    }
  },
};
</script>
