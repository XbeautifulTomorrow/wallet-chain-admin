  ...
  <div>
    <div class="searchBox">
      <div class="searchLeft">
        <el-input @keyup.enter.native="fetchWalletGasStatisticsList" clearable v-model="searchForm.idOrHash"
          placeholder="输入Hash/流水号"></el-input>

        <el-select v-model="searchForm.businessesId" filterable clearable @change="fetchWalletGasStatisticsList()"
          placeholder="全部平台">
          <el-option v-for="(item, index) in platformDrop" :key="index" :label="item.businessesName" :value="item.id">
          </el-option>
        </el-select>

        <el-select v-model="searchForm.chainName" filterable clearable @change="fetchWalletGasStatisticsList()"
          placeholder="所有网络">
          <el-option v-for="(item, index) in networkDrop" :key="index" :label="item.chainName" :value="item.chainName">
          </el-option>
        </el-select>

        <el-select v-model="searchForm.coinName" filterable clearable @change="fetchWalletGasStatisticsList()"
          placeholder="所有币种">
          <el-option v-for="(item, index) in coinDrop" :key="index" :label="item.coinName" :value="item.coinName">
          </el-option>
        </el-select>

        <el-select v-model="searchForm.type" filterable clearable @change="fetchWalletGasStatisticsList()"
          placeholder="全部行为">
        ...
        <el-button size="medium" type="primary" icon="el-icon-search"
          @click="fetchWalletGasStatisticsList">搜索</el-button>

        <el-button size="medium" type="warning" icon="el-icon-download" @click="toExportData">导出</el-button>
      </div>
      <div class="searchRight"></div>
    </div>
    <el-table border :data="gasStatisticsList" height="73vh" style="width: 99%" @sort-change="sortChange">
      ...
      <el-table-column prop="type" label="行为">
        <template slot-scope="scope">
          <span v-if="scope.row.type == '1'">归集</span>
          <span v-if="scope.row.type == '2'">提款</span>
          <span v-else-if="scope.row.type == '3'">转Gas</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="amount" label="Gas数量"></el-table-column>
...
export default {
  ...
  created() {
    ...
    this.fetchCoinDrop();
    this.fetchWalletGasStatisticsList();
  ...
  methods: {
    ...
    async fetchWalletGasStatisticsList(isSearch = true) {
      ...
      const res = await this.$http.getWalletGasStatisticsList(data);
      if (res) {
        this.gasStatisticsList = res.records;
    ...
    handleCurrentChange(val) {
      this.page = val;
      this.fetchWalletGasStatisticsList(false);
    }
  },
};
</script>
