<template>
  <div>
    <el-table border :data="coinList" height="73vh" style="width: 99%" @sort-change="sortChange">
      <el-table-column prop="id" label="币种ID"></el-table-column>
      <el-table-column prop="coinImg" label="图标">
        <template slot-scope="scope">
          <el-image style="width: 40px; height: auto;" :src="scope.row.coinImg"
            :preview-src-list="[scope.row.coinImg]"></el-image>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="defaultAggregationThresholds" label="默认归集阈值"></el-table-column>
      <el-table-column prop="mandatoryAggregationThresholds" label="强制归集阈值"></el-table-column>
      <el-table-column prop="withdrawFee" label="手续费"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    async fetchCoinConfigList(isSearch = true) {
      ...
      let res = await this.$http.getCoinConfigList(data);
      console.log(res)
      if (res) {
    ...
    handleCurrentChange(val) {
      this.page = val;
      this.fetchCoinConfigList(false);
    },
